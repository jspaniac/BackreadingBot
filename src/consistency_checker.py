...
class ConsistencyConstants:
    ...
    FERPA_VIEW_ATTEMPT_LINK = 'https://edstem.org/us/courses/{course_id}/lessons/{lesson_id}/attempts?slide={slide_id}&s={submission_id}'
    VIEW_ATTEMPT_LINK = 'https://edstem.org/us/courses/{course_id}/lessons/{lesson_id}/attempts?slide={slide_id}&email={email}'
...
class ConsistencyChecker:
    @staticmethod
    def _count_ungraded(users: List[Dict], spreadsheet: Dict[str, str]) -> Tuple[Dict[str, int], int, int]:
        """
        ...
        Params: 'users' - A list of Ed user objects for a course
                'spreadsheet' - A dictionary mapping ed user_id -> TA name. If none, section codes will be
                                used instead
        Returns: The aforementioned dictionary, the total number of students not present in the given spreadsheet,
                 and the total number of ungraded students
        """
    ...
    @staticmethod
    async def check_ungraded(ed_helper: EdHelper, url: str, spreadsheet: Optional[Dict[str, str]]=None,
                             progress_bar_update: Optional[Callable[[int, int], None]]=None) -> Tuple[Dict[str, int], int, int]:
        """
        Checks and organizes information regarding ungraded students for a given ed assignment

        Params: 'ed_helper' - A properly initialized EdHelper object with API access to the ed assignment
                'url' - The url of the ed assignment to check
                'spreadsheet' - A dictionary mapping ed user_id -> TA name. If none, section codes will be
                                used instead
                'progress_bar_update' - A function to call with incremental values that updates a user-viewable progress bar
                'attachment_url' - The url of the attachment sent with the initial user request (can be None)
        Returns: A dictionary mapping either (section | TA) -> total ungraded depending on if an attachment_url, 
                 the total number of students not present in the given spreadsheet, and the total number of
                 ungraded students
        """
    ...
    @staticmethod
    def _check_criteria(all_criteria: List[Dict], content: str) -> str:
        """
    ...
    @staticmethod
    def _find_submission_fixes(submissions: List[Dict], num_criteria: int, due_at: datetime,
                               template: bool) -> Tuple[Union[None, str], Union[None, str]]:
        """
        ...
        Params: 'submissions' - A list of Ed submission objects
                'num_criteria' - The total number of criteria that need to be filled out
        ...
        Returns: Any grading fixes that need to be made and the id of the submission. None, None if there are no issues
        """
    ...
    @staticmethod
    def _get_link(ids: List[int], user_id: int, email: str, submission_id: str, attempt_slide: bool,
                  ferpa: bool) -> str:
        """
        Creates a properly formatted ed link for a specific student submission

        Params: 'ids' - The course, lesson, and slide id for the assignment to link to
                ...
                'ferpa' - Whether or not to censor the student email from the link
    ...
    @staticmethod
    async def _find_fixes(ed_helper: EdHelper, url: str, template: Optional[bool] = False,
                          spreadsheet: Optional[Dict[str, str]] = None, 
                          progress_bar_update: Optional[Callable[[int, int], None]] = None,
                          ferpa: Optional[bool] = True) -> Tuple[Dict[str, List[Tuple[str, str]]], List[str]]:
        """
        ...
        Params: 'ed_helper' - A properly initialized EdHelper object with API access to the ed assignment
                ...
                'template' - Whether or not the grading template is expected, default False
                'spreadsheet' - A dictionary mapping ed student ID to TA name, default None
                'progress_bar_update' - A function to call with incremental values that updates a user-viewable progress bar, default None
                'ferpa' - Whether or not to censor student emails from links, default True
        Returns: A dictionary mapping (TA | link) -> (link, fixes) for all assignment that had incorrect formatting and a
                 List of links to student assignments not found in the grading spreadsheet
        """
        attempt_slide = EdHelper.is_overall_submission_link(url)
        ...
        if not attempt_slide:
            ...
            users = [(attempt['user_id'], attempt['email'], attempt['tutorial'], attempt['sourced_id'])
        ...
        for (user_id, email, section, submission_id) in users:
            if spreadsheet and str(user_id) not in spreadsheet:
                # This student isn't present in the grading spreadsheet, skip
                not_present.append(ConsistencyChecker._get_link(ids, user_id, email, submission_id, attempt_slide, ferpa))
                continue
            ...
            submissions = (ed_helper.get_challenge_submissions(user_id, challenge_id) if not attempt_slide else
                           ed_helper.get_attempt_submissions(user_id, lesson_id, slide_id, submission_id, rubric))
            if submissions is None:
                continue
            
            submission_fixes, submission_id = ConsistencyChecker._find_submission_fixes(submissions, num_criteria, due_at, template)
            if submission_fixes:
                link = ConsistencyChecker._get_link(ids, user_id, email, submission_id, attempt_slide, ferpa)
                
                key = section if spreadsheet is None else spreadsheet[str(user_id)]
    ...
    @staticmethod
    def _convert_fixes_to_list(fixes: Dict[str, Tuple[str, str]]) -> List[List[str]]:
        """
    ...
    @staticmethod
    async def check_consistency(ed_helper: EdHelper, url: str, file_name: str, template: Optional[bool]=False,
                                spreadsheet: Optional[Dict[str, str]]=None,
                                progress_bar_update=None, ferpa=True) -> Tuple[Dict[str, Tuple[str, str]], List[str], int]:
        """
        Checks and organizes information regarding grading consistency for a given ed assignment.

        Params: 'ed_helper' - A properly initialized EdHelper object with API access to the ed assignment
                'url' - The url of the ed assignment to check
                'file_name' - The name to use for the two saved .csv and .html files
        ...
        Returns: A dictionary mapping (TA | link) -> (link, fixes) for all assignment that had incorrect formatting,
                 a list of links to student assignments not found in the grading spreadsheet, and the total number of issues found
        """
        # Remove email since it mseese with ID regex
        url = ConsistencyRegex.EMAIL_REGEX.sub('', url)
        
        fixes, not_present = await ConsistencyChecker._find_fixes(ed_helper, url, template, spreadsheet, progress_bar_update, ferpa)
        if progress_bar_update:
