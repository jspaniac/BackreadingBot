...
class Database:
    """
    Represents a database which stores a variety of information about a
    specific guild necessary for the bots use
    """

    def __init__(
        self,
        file_path: Optional[str] = DB_FILE
    ):
        """
        Constructs a new database instance from the given file. If the file
        cannot be found, raises the DBFileNotFound exception

        Params: 'file_path' - Path to the file containins guild information to
                              load
        """
        ...
        try:
            ...
            raise DBFileNotFound("Database file cannot be found, unable to " +
    ...
    def _get(
        ...
        """
        Helper method that returns the guild info for a given guild_id. Note
        that the id can be an int or str
        """
        if str(guild_id) not in self.guild_to_info:
            raise GuildNotFound(f"Guild ID {guild_id} not present in the " +
                                "database")
    ...
    def get_admin(
        ...
        Returns: The discord user ID for the guild's admin (the user that
                 initialized the bot)
        """
    ...
    def get_role(
        ...
        """
        Params: 'guild_id' - The guild ID to get info for
        Returns: The discord role ID that will be pinged when a backreading
                 thread is created
        """
    ...
    def get_approval(
        ...
        """
        Params: 'guild_id' - The guild ID to get info for
        Returns: Whether or not approval is required before pushing repsonses
                 to Ed
        """
    ...
    def register(
        ...
        Params: 'guild_id' - The guild ID to import
                'guild_info' - The necessary guild information. Should be the
                               result of GuildInfo.create
        """
    ...
    def save(
        ...
        """
        Saves current information to the file the database was created with.
        If the file cannot be found, raises the DBFileNotFound exception
        """
        try:
            open(self.db_file, "w").write(json.dumps(self.guild_to_info))
        except FileNotFoundError:
            raise DBFileNotFound("Original database file can't be found, " +
                                 "unable to save")
